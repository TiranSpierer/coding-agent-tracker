...
This project scrapes weekly stats (members, visitors, contributions) from 7 Reddit coding-agent subreddits and stores them in a CSV. It runs autonomously via GitHub Actions every Saturday.
...
4. Worker re-fetches with `?solution=...`, forwarding cookies and `Referer` header from step 2
5. Reddit returns the real page with `<shreddit-subreddit-header>` containing `weekly-active-users` and `weekly-contributions` attributes
...
- `ClaudeCode`, `CLine`, `cursor`, `windsurf`, `GithubCopilot`, `google_antigravity`, `codex`

To find a subreddit's canonical name: visit `reddit.com/r/name` and check the URL after redirect.

## Data flow

1. `src/scraper.ts` calls the Worker for all 7 subreddits in parallel
